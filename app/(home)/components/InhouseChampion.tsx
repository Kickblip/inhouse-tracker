...
export default function InhouseChampion({
  ...
  return (
    <div
      className="relative w-full h-[220px] md:h-[350px] rounded-lg 
    ...
    >
      ...
      <Link href={`/p/${puuid.substring(0, 14)}`}>
        ...
        <Image
          ...
          width={1191 * 0.6}
          height={670 * 0.6}
          className="hidden md:block absolute bottom-0 right-0 translate-x-[20%] pointer-events-none select-none z-20 
          ...
          <div className="flex gap-8 items-center">
            <div className="flex flex-col space-y-1">
              ...
              <h2 className="text-2xl md:text-5xl font-extrabold">
                {((gamesWon / (gamesWon + gamesLost)) * 100 || 0).toFixed(0)}%
              </h2>
            </div>
            <div className="flex flex-col space-y-1">
              <p className="opacity-70 font-bold">Played</p>
              <h2 className="text-2xl md:text-5xl font-extrabold">{gamesWon + gamesLost}</h2>
            </div>
            <div className="flex flex-col space-y-1">
              <p className="opacity-70 font-bold">MMR</p>
              <h2 className="text-2xl md:text-5xl font-extrabold">{points}</h2>
            </div>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 z-20 py-6 px-8 font-[family-name:var(--font-geist-sans)] text-left select-none">
          ...
          <h3 className="text-4xl md:text-8xl font-extrabold">{playerName}</h3>
        </div>
      </Link>
    </div>
  )
